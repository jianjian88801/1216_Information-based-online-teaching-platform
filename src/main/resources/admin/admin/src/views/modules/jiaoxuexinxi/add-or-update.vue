    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="课程号" prop="kechenghao">
          <el-input v-model="ruleForm.kechenghao" 
              placeholder="课程号" clearable  :readonly="ro.kechenghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="课程号" prop="kechenghao">
              <el-input v-model="ruleForm.kechenghao" 
                placeholder="课程号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.kechengxiangqing = this.ruleForm.kechengxiangqing.replace(reg,'../../../springboot9qw88/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiaoxuexinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiaoxuexinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
